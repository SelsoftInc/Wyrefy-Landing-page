...
export default function ProjectDetailPage() {
  ...
  if (error || !project) {
    return (
      <div className="flex h-[calc(100vh-10rem)] flex-col items-center justify-center gap-4">
        ...
        <button type="button" onClick={() => push("/organization/projects")} className="text-blue-500 hover:underline">Back to Library</button>
      </div>
    );
  }
  ...
  return (
    <div className="flex-1 flex flex-col min-h-0 h-full">
      <AgentView project={project} onBack={() => push("/organization/projects")} />
    </div>
  );
}
